%{
    ...
    let tokens = [];
    const addToken = (type, text, line, col) => {
...
DEF_FUNC: RFUNC ID PAR_ABRE LIST_PARAMS PAR_CIERRA LLAVE_ABRE INSTRUCCIONES LLAVE_CIERRA { $$ = new DeclaracionFuncion.default($2, $4, $7, @1.first_line, @1.first_column); }
        | RFUNC ID PAR_ABRE PAR_CIERRA LLAVE_ABRE INSTRUCCIONES LLAVE_CIERRA { $$ = new DeclaracionFuncion.default($2, [], $6, @1.first_line, @1.first_column); }
;

LIST_PARAMS: LIST_PARAMS COMA PARAM { $1.push($3); $$ = $1; }
           | PARAM { $$ = [$1]; }
;

PARAM: ID TIPO { $$ = {id: $1, tipo: $2}; }
;

LLAMADA: ID PAR_ABRE LIST_ARGS PAR_CIERRA { $$ = new LlamadaFuncion.default($1, $3, @1.first_line, @1.first_column); }
       | ID PAR_ABRE PAR_CIERRA { $$ = new LlamadaFuncion.default($1, [], @1.first_line, @1.first_column); }
;

LIST_ARGS: LIST_ARGS COMA EXPRESION { $1.push($3); $$ = $1; }
         | EXPRESION { $$ = [$1]; }
;
